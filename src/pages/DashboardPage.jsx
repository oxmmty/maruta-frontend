...
const Dashboardpage = () => {
  ...
  const [totalMonthlySale, setTotalMonthlySale] = useState([]);
  const [totalMonthlyProfit, setTotalMonthlyProfit] = useState([]);
  const [totalMonthlyRatio, setTotalMonthlyRatio] = useState([]);
      ...
      try {
        ...
        const combinedData = totalProfit.data.totalProfit.map((profit) => {
          const matchingSale = totalSale.data.totalSale.find(
            (sale) => sale.year === profit.year && sale.month === profit.month
          );
          return {
            ...profit,
            profitTotalSum: matchingSale ? matchingSale.totalSum : 0,
            difference:
              (matchingSale ? matchingSale.totalSum : 0) - profit.totalSum,
          };
        });

        const totalRatio = totalProfit.data.totalProfit.map((profit) => {
          const matchingSale = totalSale.data.totalSale.find(
            (sale) => sale.year === profit.year && sale.month === profit.month
          );
          return {
            ...
            profitTotalSum: matchingSale ? matchingSale.totalSum : 0,
            売上高総利益率:
            ((profit.totalSum / ((matchingSale ? matchingSale.totalSum : 0) + profit.totalSum))*100).toFixed(2),
          };
        });

        const newCombinedData = combinedData.map((data) => {
          return {
            ...data,
            totalSum: data.difference,
          };
        });
        const newTotalRatio = totalRatio.map((data) => {
          return {
            ...data,
        ...
        setTotalMonthlySale([...totalSale.data.totalSale, ...newCombinedData]);
        setTotalMonthlyRatio(newTotalRatio)
      } catch (error) {
      ...
      <div className="flex flex-col xl:flex-row xl:h-96 justify-between gap-2 w-full">
        <div className="xl:w-1/3 h-[400px] bg-bg-light rounded-lg">
          {customerList.length > 0 && (
            <Customer totalMonthlySale={totalMonthlySale} totalMonthlyRatio={totalMonthlyRatio} />
          )}
        </div>
