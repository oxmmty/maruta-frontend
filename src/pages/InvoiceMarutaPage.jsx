  ...
  const filterData = () => {
    ...
    if (selectedCustomer) {
      filtered = filtered.filter((item) =>
        [item.顧客名].includes(selectedCustomer)
      );
    }
    ...
    <div className="w-full pl-5 pr-5 pt-3 pb-4"  ref={componentRef} >
      <div className="flex justify-between  border-b-2">
        ...
        <div className="flex items-center">
          <Title level={2} className="mb-[-5px] pr-3">
            請　求　書
          </Title>
          <Title level={5}>
            <DatePicker
        ...
        <div className="">
          ...
          <div className="flex">
            ...
            <p>
              {selectedDate
                ? dayjs(selectedDate).endOf("month").format("YYYY年MM月DD日")
                : dayjs().endOf("month").format("YYYY年MM月DD日")}
            </p>
          </div>
          <div className="flex">
            <Text className="pr-3" strong>
      ...
      <div className="flex justify-between pt-2">
        <div className="pt-4">
          <p className="pb-1">
            〒{" "}
            {selectedCustomer &&
              customers.find((item) => item.企業名略称 === selectedCustomer)[
                "郵便番号"
              ]}
          </p>
          <p className="pb-1">
        ...
        <div>
          ...
          <p>〒230-0054 横浜市鶴見区大黒埠頭22番地</p>
          <p>横浜港流通センター(YCC)7階 1703号室</p>
          <div className="flex">
            <p>【TEL】: 045-506-2901</p>
            <p>【FAX】: 045-506-2971</p>
          </div>
          <p>
            【登録番号】<span className="pl-2">: T2020001018797</span>
          </p>
          <p>
            【取引銀行】<span className="pl-2">: 横浜信用金庫　末吉支店</span>
          </p>
          <p>
            【当座預金】<span className="pl-2">: 口座番号 22742</span>
          </p>
        </div>
      </div>
      ...
      <div className="flex justify-between">
        ...
        <table className=" border-collapse border border-black h-[50px]">
          <thead>
        ...
        <table className="w-full table-auto border-collapse border border-black">
          <thead>
            <tr className="bg-gray-200 text-black">
              ...
              <th className="border border-black px-4 py-2">3軸</th>
              <th className="border border-black px-4 py-2">その他</th>
              <th className="border border-black px-4 py-2">本船名</th>
              <th className="border border-black px-4 py-2">コンテナ番号</th>
              <th className="border border-black px-4 py-2">REF No</th>
              <th className="border border-black px-4 py-2">運送料</th>
              <th className="border border-black px-4 py-2">3軸</th>
              <th className="border border-black px-4 py-2">その他</th>
              <th className="border border-black px-4 py-2">請求額</th>
              <th className="border border-black px-4 py-2">税区</th>
              <th className="border border-black px-4 py-2">扱区分</th>
            </tr>
          </thead>
          <tbody>
            {filteredData.map((item, index) => (
              <tr key={index} className="text-black">
                ...
                <td className="border border-black px-4 py-2">{item.船名}</td>
                <td className="border border-black px-4 py-2">
                  {item.コンテナNo}
                </td>
                <td className="border border-black px-4 py-2">
                ...
                <td className="border border-black px-4 py-2">
                  {item.基本料金1 + item["3軸料金1"] + item.その他費用
                    ? "¥"
                    : ""}
                  {item.基本料金1 + item["3軸料金1"] + item.その他費用}
                </td>
                ...
                <td className="border border-black px-4 py-2">{item.区分}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default InvoiceMarutaPage;
