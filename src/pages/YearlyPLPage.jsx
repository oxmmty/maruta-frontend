...
const YearlyPLPage = () => {
  ...
  const [filteredDataOperatingProfit, setFilteredDataOperatingProfit] =
    useState(0);
  const [filteredDataOperatingIncome, setFilteredDataOperatingIncome] =
    useState(0);
  const [filteredDataOperatingExpenses, setFilteredDataOperatingExpenses] =
    useState(0);
  const [filteredDataOrdinaryProfit, setFilteredDataOrdinaryProfit] =
    useState(0);
  const [filteredDataExtraordinaryProfit, setFilteredDataExtraordinaryProfit] =
    useState(0);
  const [filteredDataExtraordinaryLosses, setFilteredDataExtraordinaryLosses] =
    useState(0);
  ...
  const [totalMActualOperatingProfit, setTotalMActualOperatingProfit] =
    useState(0);
  const [totalMActualOperatingIncome, setTotalMActualOperatingIncome] =
    useState(0);
  const [totalMActualOperatingExpenses, setTotalMActualOperatingExpenses] =
    useState(0);
  const [totalMActualOrdinaryProfit, setTotalMActualOrdinaryProfit] =
    useState(0);
  const [totalMActualExtraordinaryProfit, setTotalMActualExtraordinaryProfit] =
    useState(0);
  const [totalMActualExtraordinaryLosses, setTotalMActualExtraordinaryLosses] =
    useState(0);
  ...
  const [
    totalMActualRatioOperatingProfit,
    setTotalMActualRatioOperatingProfit,
  ] = useState(0);
  const [
    totalMActualRatioOperatingIncome,
    setTotalMActualRatioOperatingIncome,
  ] = useState(0);
  const [
    totalMActualRatioOperatingExpenses,
    setTotalMActualRatioOperatingExpenses,
  ] = useState(0);
  const [totalMActualRatioOrdinaryProfit, setTotalMActualRatioOrdinaryProfit] =
    useState(0);
  const [
    totalMActualRatioExtraordinaryProfit,
    setTotalMActualRatioExtraordinaryProfit,
  ] = useState(0);
  const [
    totalMActualRatioExtraordinaryLosses,
    setTotalMActualRatioExtraordinaryLosses,
  ] = useState(0);
  const [totalMActualRatioPretaxProfit, setTotalMActualRatioPretaxProfit] =
    useState(0);
  const [totalMActualRatioCorporateTax, setTotalMActualRatioCorporateTax] =
    useState(0);
  const [totalMActualRatioNetIncome, setTotalMActualRatioNetIncome] =
    useState(0);
  ...
  const [totalPlanOperatingExpenses, setTotalPlanOperatingExpenses] =
    useState(0);
  const [totalPlanOrdinaryProfit, setTotalPlanOrdinaryProfit] = useState(0);
  const [totalPlanExtraordinaryProfit, setTotalPlanExtraordinaryProfit] =
    useState(0);
  const [totalPlanExtraordinaryLosses, setTotalPlanExtraordinaryLosses] =
    useState(0);
  ...
  const [totalRatioOperatingExpenses, setTotalRatioOperatingExpenses] =
    useState(0);
  const [totalRatioOrdinaryProfit, setTotalRatioOrdinaryProfit] = useState(0);
  const [totalRatioExtraordinaryProfit, setTotalRatioExtraordinaryProfit] =
    useState(0);
  const [totalRatioExtraordinaryLosses, setTotalRatioExtraordinaryLosses] =
    useState(0);
  ...
  const filterOrderDataByYear = (selectedYear, orderData, classification) => {
    ...
    const filterOrderData = orderData.reduce((acc, item) => {
      ...
      if (!acc[year]) {
        acc[year] = {
          ...
          利用運送原価: 0,
          [`${year}mActual`]: 0,
          [`${year}mComparedActual`]: 0,
          [`${year}mComparedRatio`]: "100%",
          [`${year}SalesRatio`]: "0%",
        };
      }
      ...
      acc[year].利用運送原価 += item.下払その他費用1 + item.下払高速費1;

      if (classification === "運送売上") {
        acc[year][`${year}mActual`] += acc[year].運送売上;
      } else if (classification === "利用運送") {
        acc[year][`${year}mActual`] += acc[year].利用運送;
      } else if (classification === "自社原価") {
        acc[year][`${year}mActual`] += acc[year].自社原価;
      } else if (classification === "利用運送原価（下払い）") {
        acc[year][`${year}mActual`] += acc[year].利用運送原価;
      } else if (classification === "自社原価：運送売上") {
        acc[year][`${year}mActual`] += acc[year].運送売上 - acc[year].自社原価;
      } else if (classification === "利用運送原価：利用運送") {
        acc[year][`${year}mActual`] +=
          acc[year].利用運送 - acc[year].利用運送原価;
      } else if (
      ...
      totalMActualSum += acc[year][`${year}mActual`];

      return acc;
    }, {});

    let arrayFromEntries = Object.entries(filterOrderData).map(
      ([year, values]) => ({
        year,
        ...values,
      })
    ...
    setArrYear(Object.keys(filterOrderData).map(Number));

    const finalData = arrayFromEntries.map(({ year, ...rest }) => ({
      year,
      ...rest,
      ...(filterOrderData[year] || {
        運送売上: 0,
        利用運送: 0,
        [`${year}mActual`]: 0,
        [`${year}mComparedActual`]: 0,
        [`${year}mComparedRatio`]: "0%",
        [`${year}SalesRatio`]: "0%",
      }),
    }));
  ...
  const filterAccountplDataByYear = (selectedYear, accountplData, classification) => {
    const filterAccountplData = accountplData
      ...
      //   return String(itemYear) == String(selectedYear.year());
      // })
      .reduce((acc, item) => {
        const year = item.accountYear;
        ...
        acc[year].accountAmount += item.accountAmount;
        ...
        if (
          ...
          acc[year][`${year}mActual`] += item.accountAmount;
        } else if (
          classification === "その他原価" &&
          item.accountList === "その他原価"
        ) {
    ...
    const totalMActualSum = finalData.reduce((sum, monthData) => {
      return sum + monthData[`${monthData.year}mActual`];
    }, 0);
  ...
  const calculateTotalSalesByMonth = (data1, data2, data3) => {
    return data1.map((item, index) => {
      ...
      const totalActual =
        item[`${year}mActual`] +
        (data2[index] ? data2[index][`${year}mActual`] : 0) +
        (data3[index] ? data3[index][`${year}mActual`] : 0);
      return {
        year,
        [`${year}mActual`]: totalActual,
        [`${year}mComparedActual`]: 0,
        [`${year}mComparedRatio`]: "0%",
        [`${year}SalesRatio`]: "100%",
      };
  ...
  const calculateGrossProfitByMonth = (
    ...
    return data1.map((item, index) => {
      ...
      const totalGrossProfit =
        ...
        (data2[index] ? data2[index][`${year}mActual`] : 0) +
        (data3[index] ? data3[index][`${year}mActual`] : 0) -
        (data4[index] ? data4[index][`${year}mActual`] : 0) -
        (data5[index] ? data5[index][`${year}mActual`] : 0) -
        (data6[index] ? data6[index][`${year}mActual`] : 0);
      return {
        year,
        [`${year}mActual`]: totalGrossProfit,
        [`${year}mComparedActual`]: 0,
        [`${year}mComparedRatio`]: "0%",
        [`${year}SalesRatio`]: "0%",
      };
  ...
  const calculatePLSumByMonth = (
    ...
    return data1.map((item, index) => {
      ...
      const PLSum =
        ...
        (data5[index] ? data5[index][`${year}mActual`] : 0) +
        (data6[index] ? data6[index][`${year}mActual`] : 0);
      +(data7[index] ? data2[index][`${year}mActual`] : 0) +
        (data8[index] ? data3[index][`${year}mActual`] : 0) +
        (data9[index] ? data4[index][`${year}mActual`] : 0) +
        (data10[index] ? data5[index][`${year}mActual`] : 0) +
        (data11[index] ? data6[index][`${year}mActual`] : 0);
      +(data12[index] ? data2[index][`${year}mActual`] : 0) +
        (data13[index] ? data3[index][`${year}mActual`] : 0) +
        (data14[index] ? data4[index][`${year}mActual`] : 0) +
        (data15[index] ? data5[index][`${year}mActual`] : 0);
      return {
        year,
        [`${year}mActual`]: PLSum,
        [`${year}mComparedActual`]: 0,
  ...
  const calculateOrdinaryProfitByMonth = (data1, data2, data3) => {
    return data1.map((item, index) => {
      ...
      const ordinaryProfit =
        item[`${year}mActual`] +
        (data2[index] ? data2[index][`${year}mActual`] : 0) -
        (data3[index] ? data2[index][`${year}mActual`] : 0);
      return {
        year,
        [`${year}mActual`]: ordinaryProfit,
        [`${year}mComparedActual`]: 0,
        [`${year}mComparedRatio`]: "0%",
        [`${year}SalesRatio`]: "0%",
      };
  ...
  const calculateIncomebeforeProfitByMonth = (data1, data2, data3) => {
    return data1.map((item, index) => {
      ...
      return {
        year,
        [`${year}mActual`]: incomebeforeProfit,
        [`${year}mComparedActual`]: 0,
        [`${year}mComparedRatio`]: "0%",
        [`${year}SalesRatio`]: "0%",
      };
  ...
  useEffect(() => {
    ...
    setTotalPlanOrdinaryProfit(
      totalPlanOperatingProfit +
        totalPlanOperatingIncome -
        totalPlanOperatingExpenses
    );
    setTotalPlanExtraordinaryProfit(totalPlanExtraordinaryProfit);
    setTotalPlanExtraordinaryLosses(totalPlanExtraordinaryLosses);
    setTotalPlanPretaxProfit(
      totalPlanOrdinaryProfit +
        totalPlanExtraordinaryProfit -
        totalPlanExtraordinaryLosses
    );
    setTotalMActualNetIncome(
      totalMActualPretaxProfit - totalMActualCorporateTax
    );
    setTotalPlanCorporateTax(totalPlanCorporateTax);
    setTotalPlanNetIncome(totalPlanPretaxProfit - totalPlanCorporateTax);
  }, [totalPlan9, totalPlan13, totalPlan4, totalPlan8, totalPlan1, totalPlan2, totalPlan3,  totalPlanOrdinaryProfit, totalPlanExtraordinaryProfit, totalPlanExtraordinaryLosses]);

  useEffect(() => {
    setTotalRatio9(
      `${(((totalPlan4 - totalPlan8) / totalPlan8) * 100).toFixed(2)}%`
    );
    setTotalRatio10(
      `${(((totalPlan1 - totalPlan5) / totalPlan1) * 100).toFixed(2)}%`
    );
    setTotalRatio11(
      `${(((totalPlan2 - totalPlan6) / totalPlan1) * 100).toFixed(2)}%`
    );
    setTotalRatio12(
      `${(((totalPlan1 - totalPlan5) / totalPlan1) * 100).toFixed(2)}%`
    );
    ...
    setTotalRatioOperatingProfit(
      `${((totalPlanOperatingProfit / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioOperatingIncome(
      `${((totalPlanOperatingIncome / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioOperatingExpenses(
      `${((totalPlanOperatingExpenses / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioOrdinaryProfit(
      `${((totalPlanOrdinaryProfit / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioExtraordinaryProfit(
      `${((totalPlanExtraordinaryProfit / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioExtraordinaryLosses(
      `${((totalPlanExtraordinaryLosses / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioPretaxProfit(
      `${((totalPlanPretaxProfit / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioCorporateTax(
      `${((totalPlanCorporateTax / totalPlan4) * 100).toFixed(2)}%`
    );
    setTotalRatioNetIncome(
      `${((totalPlanNetIncome / totalPlan4) * 100).toFixed(2)}%`
    );
  }, [
    totalPlan13,
    totalPlan4,
    ...
    totalPlanOperatingExpenses,
    totalPlanOrdinaryProfit,
    totalPlanPretaxProfit,
    totalPlanCorporateTax,
    totalPlanNetIncome,
  ]);
  ...
  useEffect(() => {
    ...
    setTotalMActualOrdinaryProfit(
      totalMActualOperatingProfit +
        totalMActualOperatingIncome -
        totalMActualOperatingExpenses
    );
    setTotalMActualPretaxProfit(
      totalMActualOrdinaryProfit +
        totalMActualExtraordinaryProfit -
        totalMActualExtraordinaryLosses
    );
    setTotalMActualNetIncome(
      totalMActualPretaxProfit - totalMActualCorporateTax
    );
  ...
  useEffect(() => {
    setTotalMActualRatio6(
      `${((totalMActual6 / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio5(
      `${((totalMActual5 / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio2(
      `${((totalMActual2 / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio1(
      `${((totalMActual1 / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio9(
      `${(((totalMActual4 - totalMActual8) / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio10(
      `${(((totalMActual4 - totalMActual5) / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio11(
      `${(((totalMActual4 - totalMActual6) / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatio12(
      `${(((totalMActual4 - totalMActual5) / totalMActual4) * 100).toFixed(2)}%`
    );
    ...
    setTotalMActualRatioPLSum(
      `${((totalMActualPLSum / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioOperatingProfit(
      `${((totalMActualOperatingProfit / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioOperatingIncome(
      `${((totalMActualOperatingIncome / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioOperatingExpenses(
      `${((totalMActualOperatingExpenses / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioOrdinaryProfit(
      `${((totalMActualOrdinaryProfit / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioExtraordinaryProfit(
      `${((totalMActualExtraordinaryProfit / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioExtraordinaryLosses(
      `${((totalMActualExtraordinaryLosses / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioPretaxProfit(
      `${((totalMActualPretaxProfit / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioCorporateTax(
      `${((totalMActualCorporateTax / totalMActual4) * 100).toFixed(2)}%`
    );
    setTotalMActualRatioNetIncome(
      `${((totalMActualNetIncome / totalMActual4) * 100).toFixed(2)}%`
    );
  }, [
    totalMActual13,
    totalPlan4,
    ...
    totalMActualExtraordinaryProfit,
    totalMActualOrdinaryProfit,
    totalMActualPretaxProfit,
    totalMActualCorporateTax,
  ]);
  useEffect(() => {
    ...
    const allFilteredData = [
      ...
      filteredDataPretaxProfit,
      filteredDataCorporateTax,
      filteredDataNetIncome,
    ];

    filteredData4.forEach((e, i) => {
      allFilteredData.forEach((filteredData) => {
        filteredData[i][`${e.year}SalesRatio`] = `${(
          (filteredData[i][`${e.year}mActual`] / e[`${e.year}mActual`]) *
          100
        ).toFixed(2)}%`;
      });
    });
      ...
      children: [
        {
          ...
          render: (text, record) => {
            ...
            if (record.type === 1 && planEdit) {
              return (
                <Form.Item
                  name={`${record.classification}-${
                    selectedYear?.$y || new Date().getFullYear()
                  }-${year}mActual`}
                  initialValue={monthData ? monthData[`${year}mActual`] : "-"}
                >
                  <input
                    type="number"
                    defaultValue={monthData ? monthData[`${year}mActual`] : "-"}
                  />
                </Form.Item>
              );
            }
            return monthData ? monthData[`${year}mActual`] : "-";
          },
        },
        {
          title: "前比",
          dataIndex: `monthlyData[${key}].mComparedActual`,
          key: `${year}mComparedActual`,
          align: "center",
          render: (text, record) => {
            const monthData = record.monthlyData[key];
            const prevMonthData = record.monthlyData[key - 1]?.year
              ? record.monthlyData[key - 1]
              : null;
            if (!monthData) return "-";
            if (key === 0) return monthData[`${year}mActual`];
            return prevMonthData
              ? monthData[`${record.monthlyData[key].year}mActual`] -
                  prevMonthData[`${record.monthlyData[key - 1].year}mActual`]
              : "-";
          },
        },
        {
          ...
          render: (text, record) => {
            ...
            return prevMonthData
              ? `${(
                  (monthData[`${record.monthlyData[key].year}mActual`] /
                    prevMonthData[
                      `${record.monthlyData[key - 1].year}mActual`
                    ]) *
                  100
                ).toFixed(2)}%`
              : "-";
        ...
        },
        {
          ...
          render: (text, record) => {
            if (key === 0) return "-";
            const monthData = record.monthlyData[key];
            return monthData ? monthData[`${year}SalesRatio`] : "-";
          },
        },
      ],
    })),
