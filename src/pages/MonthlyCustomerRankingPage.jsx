...
const MonthlyCustomerRankingPage = () => {
  ...
  let averageLastMonthPrice = Math.floor(
    selectedYearLastMonthPrice.reduce((sum, item) => sum + item.Price, 0) /
      lastMonthBusinessDaysCount
  );
  let averageSelectedMonthPrice = Math.floor(
    selectedYearThisMonthPrice.reduce((sum, item) => sum + item.Price, 0) /
      selectedMonthBusinessDaysCount
  );
  let totalLastMonthPrice = selectedYearLastMonthPrice.reduce(
    (sum, item) => sum + item.Price,
    0
  );
  let totalSelectedMonthPrice = selectedYearThisMonthPrice.reduce(
    (sum, item) => sum + item.Price,
    0
  );
  ...
  const averageDataSource = {
    ...
    ratio:
      averageSelectedMonthPrice && averageLastMonthPrice
        ? (
            (averageSelectedMonthPrice / averageLastMonthPrice) * 100 || 0
          ).toFixed(2) + "%"
        : "0%",
  };
  const TotalDataSource = {
    ...
    ratio:
      totalSelectedMonthPrice && totalLastMonthPrice
        ? ((totalSelectedMonthPrice / totalLastMonthPrice) * 100 || 0).toFixed(
            2
          ) + "%"
        : "0%",
  };
  ...
  const combined = customers
    .map((customer, index) => {
      return {
        rank:
          selectedYearThisMonthPrice
            .map((price, i) => ({ price: price.Price, index: i }))
        ...
        ratio:
          selectedYearThisMonthPrice[index]?.Price && totalSelectedMonthPrice
            ? (
                (selectedYearThisMonthPrice[index]?.Price /
        ...
        lastRank: (() => {
          ...
          } else if (lastMonthRank < currentRank) {
            return `${lastMonthRank}➘`;
          } else {
            return `${lastMonthRank}➚`;
          }
    ...
    .sort((a, b) => b[selectedYearThisMonth] - a[selectedYearThisMonth])
  ...
  const columns = [
...

    {
      ...
      align: "center",
      sorter: (a, b) => a[selectedYearThisMonth] - b[selectedYearThisMonth],
      render: (text) => {
        return {
          props: {
...
export default MonthlyCustomerRankingPage;
